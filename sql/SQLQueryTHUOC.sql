      ...
      CREATE PROC usp_DonOnlineChiTiet
            @Madonhang VARCHAR(10)
      AS 
            SELECT TENTHUOC, SOLUONG, DONVITINH, THANHTIEN FROM DONHANGONLINE D, DONXUATONLINECHITIET DCT, THUOC
            WHERE D.MADONHANG = DCT.MADONHANG
 ...
 --tạo trigger khi thêm dữ liệu vào bảng GIOHANG
	...
	--lưu lại thu tiền tổng hợp
      ...
      SELECT D.NGAYLAP, MADONHANG, K.TENKHACH, N.TENNV, DATHANHTOAN AS THUTIEN FROM DONHANGXUAT D, KHACHHANG K, NHANVIEN N
      WHERE K.MAKH = D.MAKH
      AND N.MANV = D.MANV
      AND DATHANHTOAN != 0
      GO
	...
	--Lưu lại chi tiền tổng hợp
      ...
      SELECT D.NGAYLAP, MADONHANG, D.MANCC, N.TENNCC, D.DATHANHTOAN AS CHITIEN FROM DONHANGNHAP D, NHACUNGCAP N
      WHERE D.MANCC = N.MANCC
      AND DATHANHTOAN != 0
      GO
